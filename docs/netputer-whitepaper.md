...
Every network is a single `.npf` file — the Netputer Package Format — containing the complete architecture and trained weights. Load it. Run it. No separate model definition. No runtime dependencies. Crucially, `.npf` is a container, not an embedded semantic policy: it records layer records, parameters, weights, and biases, but it does not require the format itself to decide whether adjacent layers are shape-consistent or to enforce one framework's operator semantics over another's. Those checks belong to the runtime that loads the file, or to optional helper tools used during development. (See Appendix B for format details.)
...
The Netputer Package Format is an open flat binary container. Any tool that produces a valid `.npf` file can target Netputer — the format is the contract, not any particular toolchain. That is deliberate: `.npf` defines how architecture records, weights, biases, and checksums are stored, while semantic-level checks such as shape consistency or operator interpretation live outside the format, in runtimes and optional helper tools.
...
The full specification is maintained as a separate document: **NPF Format Specification v1.4**.
